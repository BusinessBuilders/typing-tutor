...
export class OpenAIService implements IAIProvider {
  ...
  initialize(config: AIConfig): void {
    ...
    if (config.apiKey) {
      this.baseURL = config.apiKey.startsWith('sk-')
  ...
  private async makeRequest(prompt: string, systemPrompt?: string): Promise<string> {
    if (!this.isConfigured()) {
      throw new AIProviderError('OpenAI not configured', this.provider);
    }
  ...
  async generateScene(request: ContentGenerationRequest): Promise<ContentGenerationResponse> {
    const systemPrompt = `You are a creative assistant for a typing tutor designed for children with autism. Generate vivid but clear scene descriptions for story-based typing practice. Use concrete, sensory details.`;
  ...
  async generateEncouragement(request: EncouragementRequest): Promise<EncouragementResponse> {
    const systemPrompt = `You are an encouraging assistant for a typing tutor designed for children with autism. Provide specific, genuine praise. Be direct and sincere.`;
    ...
    prompt += `Be specific about what they did well.`;
  ...
  async evaluateTyping(request: TypingEvaluationRequest): Promise<TypingEvaluationResponse> {
    const systemPrompt = `You are a supportive tutor for children with autism. Provide constructive, positive feedback on typing practice.`;
...
Keep each point to one short sentence.`;
  ...
  async extractImageKeywords(content: string): Promise<string[]> {
    const systemPrompt = `Extract 2-3 concrete nouns from the text that would make good image search keywords. Return ONLY the keywords separated by commas.`;
  ...
  async generateAchievementMessage(achievementType: string, context: string): Promise<string> {
    const systemPrompt = `Generate a celebratory message for an achievement in a typing tutor for children with autism. Keep it brief and genuine.`;
