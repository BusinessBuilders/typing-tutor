...
export class ClaudeService implements IAIProvider {
  readonly provider: AIProviderType = 'claude';
  model: string;
  ...
  async validateApiKey(): Promise<boolean> {
    ...
    try {
      ...
      const response = await this.makeRequest('Hello', 'You are a helpful assistant.');
      return !!response;
    } catch (error: any) {
  ...
  private async makeRequest(prompt: string, systemPrompt?: string): Promise<string> {
    if (!this.isConfigured()) {
      throw new AIProviderError('Claude not configured', this.provider);
    }
  ...
  async generateWord(request: ContentGenerationRequest): Promise<ContentGenerationResponse> {
    ...
    prompt += ` Respond with ONLY the word, nothing else. No explanations or punctuation.`;
  ...
  async generateScene(request: ContentGenerationRequest): Promise<ContentGenerationResponse> {
    ...
    prompt += ` Use concrete, sensory language. Make it engaging but predictable. Respond with ONLY the scene description, no preamble.`;
